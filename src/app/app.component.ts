...
export class AppComponent implements OnInit, OnDestroy {
  ...
  setupDeepLinking() {
    ...
    App.addListener('appUrlOpen', (data: { url: string }) => {
      ...
      console.log('[Deep link received] URL contains newlines:', data.url.includes('%0A'));
      console.log('[Deep link received] URL contains spaces:', data.url.includes(' '));
  ...
  private async processPaymentWithToken(token: string, orderId: string) {
    ...
    try {
      ...
      const tokenLog = isDev ? token : `${token.substring(0, 5)}...${token.substring(token.length - 3)}`;
      ...
      console.log(`[Payment Flow] Waiting ${INITIAL_DELAY / 1000} seconds before first status check...`);
      ...
      while (retryCount < MAX_RETRIES) {
        ...
        try {
          ...
          if (response?.data?.resultText?.includes('QUERY_FAILED') || 
          ...
          else if (response?.data?.status === 'FAILED' || response?.status >= 400) {
            throw new Error(response?.data?.resultText || 'Payment failed');
          }
        } catch (error: any) {
          console.error(`[Payment Flow] Error in polling attempt ${retryCount}:`, error.message || error);
          lastError = error;
          ...
          if (retryCount >= MAX_RETRIES) {
            let errorMessage = 'Payment processing timed out. Please check your transaction status.';
        ...
        if (retryCount < MAX_RETRIES) {
          console.log(`[Payment Flow] Waiting ${RETRY_DELAY / 1000} seconds before next attempt...`);
          await new Promise(resolve => setTimeout(resolve, RETRY_DELAY));
        }
      }

      // If we've exhausted all retries
      throw new Error('Payment processing timed out. Please check your transaction status.');
      ...
      const errorMessage = error?.message || 'An error occurred while processing your payment. Please try again.';
      ...
      if (loading) {
        await loading.dismiss().catch(err => 
    ...
    try {
      ...
      const getParam = (name: string, url: string): string | null => {
        ...
        try {
          ...
          console.error(`[Payment Flow] Error extracting parameter '${name}':`, e);
          return null;
        }
      };
      
      // Try different parameter name variations
      const token = getParam('token', cleanedUrl) || 
                   getParam('tok', cleanedUrl) ||
                   getParam('paymentToken', cleanedUrl);
                  
      const orderId = getParam('orderId', cleanedUrl) || 
      ...
      if (!token || !orderId) {
        ...
        const lastResortToken = (url.match(/[?&](?:token|tok|paymentToken)=([^&\s]+)/i) || [])[1];
        const lastResortOrderId = (url.match(/[?&](?:order[_-]?id|orderId)=([^&\s]+)/i) || [])[1];
        
        if (lastResortToken && lastResortOrderId) {
          console.warn('[Payment Flow] Using last resort parameter extraction');
          return this.processPaymentWithToken(lastResortToken, lastResortOrderId);
        }
        
        this.global.showToast('Invalid payment response - missing required parameters', 'danger', 'bottom', 3000);
        this.router.navigate(['/tabs/home']);
        return;
      }
      ...
      this.global.showToast('Error processing payment. Please try again.', 'danger', 'bottom', 3000);
      this.router.navigate(['/tabs/home']);
    }
  }
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        try {
          ...
          console.error('[Payment Flow] Invalid pendingTransaction JSON:', pendingTx, e);
        ...
        // Show error message
        this.global.showToast(
          result.error || 'Payment verification failed. Please check your payment and try again.',
          'danger',
          'bottom',
          3000
        );
  ...
  private async simulatePaymentCheck(orderId: string, token: string): Promise<{ success: boolean; data?: any; error?: string }> {
  ...
  private async processPendingTransaction() {
    try {
      ...
      this.global.showToast('Error processing transaction', 'danger', 'bottom', 3000);
      this.router.navigate(['/tabs/home']);
    }
  }
